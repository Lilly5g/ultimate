...
public class ReqCheckAnnotator implements IReq2PeaAnnotator {
	...
	public static boolean RTI_PRECHECK = true;
	...
	private List<Statement> genChecksRTInconsistency(final BoogieLocation bl) {
		...
		List<Entry<PatternType<?>, PhaseEventAutomata>> consideredAutomata = new ArrayList<>();
		if (RTI_PRECHECK) {
			consideredAutomata = mRtInconcistencyConditionGenerator.getRelevantRequirementsPreCheck(mReqPeas);
		} else {
			consideredAutomata = mRtInconcistencyConditionGenerator.getRelevantRequirements(mReqPeas);
		}
